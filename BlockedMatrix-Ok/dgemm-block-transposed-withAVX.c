...
static void do_block (int lda, int M, int N, int K, double* A, double* B, double* C)
{
  ...
  {  /* For each column j of B */
    for (int j = 0; j < N; ++j) 
    {
      ...
      for( int k = 0; k < col_reduced; k+=16 )
      {  
   	//cij += T[i*K+k] * B[j*lda+k];	
        // cij += A[i+ M*k]+B[j*N+k];
        ymm8  = _mm256_loadu_pd(&T[i*K+k]);
	ymm9  = _mm256_loadu_pd(&T[i*K+k+4]);
	ymm10 = _mm256_loadu_pd(&T[i*K+k+8]);
	ymm11 = _mm256_loadu_pd(&T[i*K+k+12]);
	...
	_mm256_storeu_pd(scratchpad, ymm0);
      ...
      for( int k = col_reduced; k < col_reduced32; k+=8)
      {  
   	//cij += T[i*K+k] * B[j*lda+k];	
        // cij += A[i+ M*k]+B[j*N+k];
        ymm8 = _mm256_loadu_pd(&T[i*K+k]);
 ...
 // }
  ...
  {  	
	/* For each block-column of B */
		...
		//double * temp2 = malloc(N*K*sizeof(double));
   		//memcpy(temp2, B+k+jLda, N*K*sizeof(double));
		for (int i = 0; i < lda; i += BLOCK_SIZE)
 	 	...
 	 	//	double * temp1 = malloc(M*K * sizeof(double));
   		//	memcpy(temp1, A+i+kLda, M*K * sizeof(double));
